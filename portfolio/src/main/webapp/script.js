...
function addRandomFact() {
  const facts = [
    ...
    'Ankita has two brothers at Google :O', 'Ankita loves collecting socks',
...
function loadComments() {
  fetch('/data?num-results=' + getCommentLimit())
...
/**
 * Creates an element that represents a comment through manual string
 * concatenation
 */
...
function createMap() {
  const map = new google.maps.Map(
      document.getElementById('map'), {center: {lat: 23, lng: -42}, zoom: 1.3});
  ...
  fetch('/restaurant-data')
      ...
      .then((restaurants) => {
        ...
        restaurantMap.addListener('click', function(e) {
          user_marker =
              placeMarkerAndPanTo(e.latLng, restaurantMap, restaurants);
        });
      });
}
...
function getBio(restaurant) {
  ...
  return restaurant.name + ', located in ' + restaurant.city + ', ' +
      restaurant.region + '. It serves ' + restaurant.cuisine +
      ' food. \n It is ' + priceLevel;
}
...
function distance(latLng, restaurant) {
  return Math.pow(
      Math.pow((latLng.lat() - restaurant.lat), 2) +
          Math.pow((latLng.lng() - restaurant.lng), 2),
      0.5);
}
...
function drawChart() {
  ...
  // For each orgchart box, we display name and position, and hoverable username
  // access
  data.addRows([
    [
      {
        'v': 'Sundar',
        'f': 'Sundar<div style="color:red; font-style:italic">CEO Man!</div>'
      },
      '', 'sundar@'
    ],
    [
      {
        'v': 'Thomas',
        'f':
            'Thomas<div style="color:red; font-style:italic">Google Cloud Ninja</div>'
      },
      'Sundar', 'tkurian@'
    ],
    [
      {
        'v': 'Javier',
        'f':
            'Javier<div style="color:red; font-style:italic">GSuite Legend</div>'
      },
      'Thomas', 'jsoltero@'
    ],
    [
      {
        'v': 'Aparna',
        'f':
            'Aparna<div style="color:red; font-style:italic">Apps Engineering Goddess</div>'
      },
      'Javier', 'apappu@'
    ],
    [
      {
        'v': 'Jim',
        'f':
            'Jim<div style="color:red; font-style:italic">DRIVEr of Workflow</div>'
      },
      'Aparna', 'jimgiles@'
    ],
    [
      {
        'v': 'Bryan',
        'f':
            'Bryan<div style="color:red; font-style:italic">DRIVEr of Backend everything!</div>'
      },
      'Jim', 'bryanv@'
    ],
    [
      {
        'v': 'Till',
        'f':
            'Till<div style="color:red; font-style:italic">Aaron\'s old boss</div>'
      },
      'Bryan', 'till@'
    ],
    [
      {
        'v': 'Lakshmanan',
        'f':
            'Lakshmanan<div style="color:red; font-style:italic">Wipeout, most importantly</div>'
      },
      'Till', 'lakshmanans@'
    ],
    [
      {
        'v': 'Saurabh',
        'f':
            'Saurabh<div style="color:red; font-style:italic">Wipeout wipeout wipeout!! woohoo!</div>'
      },
      'Lakshmanan', 'saur@'
    ],
    [
      {
        'v': 'Christian',
        'f':
            'Christian<div style="color:red; font-style:italic">Wipeout wipeout wipeout!! woohoo!</div>'
      },
      'Lakshmanan', 'cjwert@'
    ],
    [
      {
        'v': 'Adrian',
        'f': 'Adrian<div style="color:red; font-style:italic">smol intern</div>'
      },
      'Saurabh', 'amanhey@'
    ],
    [
      {
        'v': 'Ankita',
        'f':
            'Ankita<div style="color:green; font-style:italic">smol intern</div>'
      },
      'Saurabh', 'mitraan@'
    ],
    [
      {
        'v': 'Danya',
        'f': 'Danya<div style="color:red; font-style:italic">smol intern</div>'
      },
      'Saurabh', 'danyagao@'
    ],
  ]);
...
/**
   Draws the tree; sentiment value of a phrase ranges from 0(something Ankita
   is not at all proud of) to 10(something Ankita IS proud of!)
 */
function drawTree() {
  ...
  var chart =
      new google.visualization.WordTree(document.getElementById('wordtree'));
